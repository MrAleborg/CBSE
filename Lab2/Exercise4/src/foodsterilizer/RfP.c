...
printf("Now we must keep product 5 seconds at 140\n");
			int quit=-1;
			clock_t timeout = clock() + 5*CLOCKS_PER_SEC;
			while(clock()<timeout && quit!=1){
				//waiting
				switch(CALL(REQUIRED.d_a, raiseAlarm)){
				case -1: //ERROR
					printf("Delay Alarm component raised an alarm!!!\n");
                                        quit=0;
					break;
				case 0: //temp fell but less than 5 sec
					printf("Temperature fell under 140 less than 5 sec, restart procedure...\n");
